...
def PlotBowSim(alphas, base):
    ...
    for alpha in alphas:
        ...
        # Draw the strings
        plt.plot((positions[0][0], positions[1][0]), (positions[0][1], positions[1][1]), color="blue", zorder=1)

        # Draw left lever
        plt.plot((base["posx_x"], positions[0][0]), ([base["posx_y"], positions[0][1]]), color="black")
        # Draw right lever
        plt.plot((base["posy_x"], positions[1][0]), ([base["posy_y"], positions[1][1]]), color="black")
    ...
    string_diameter = 1
    string_e = plt.Circle((-17.25, 33.85), string_diameter / 2, fill=True, color="red", zorder=1)
    string_a = plt.Circle((-5.94, 37.53), string_diameter / 2, fill=True, color="red", zorder=1)
    string_d = plt.Circle((5.94, 37.53), string_diameter / 2, fill=True, color="red", zorder=1)
    string_g = plt.Circle((17.25, 33.85), string_diameter / 2, fill=True, color="red", zorder=1)
